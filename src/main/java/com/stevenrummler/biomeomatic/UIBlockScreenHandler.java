...
public class UIBlockScreenHandler extends ScreenHandler {
    ...
    public UIBlockScreenHandler(int syncId, PlayerInventory inventory, ScreenHandlerContext context) {
        super(ScreenHandlerType.CARTOGRAPHY_TABLE, syncId);
        this.inventory = new SimpleInventory(2) {
            public void markDirty() {
                UIBlockScreenHandler.this.onContentChanged(this);
                super.markDirty();
            }
        };
        ...
        this.addSlot(new Slot(this.resultInventory, 2, 145, 39) {
            ...
            public void onTakeItem(PlayerEntity player, ItemStack stack) {
                ((Slot)UIBlockScreenHandler.this.slots.get(0)).takeStack(1);
                ((Slot)UIBlockScreenHandler.this.slots.get(1)).takeStack(1);
                stack.getItem().onCraft(stack, player.world, player);
                context.run((world, pos) -> {
                    long l = world.getTime();
                    if (UIBlockScreenHandler.this.lastTakeResultTime != l) {
                        world.playSound((PlayerEntity)null, pos, SoundEvents.UI_CARTOGRAPHY_TABLE_TAKE_RESULT, SoundCategory.BLOCKS, 1.0F, 1.0F);
                        UIBlockScreenHandler.this.lastTakeResultTime = l;
                    }
    ...
    public boolean canUse(PlayerEntity player) {
        return canUse(this.context, player, BiomeOMatic.UIBLOCK);
    }
